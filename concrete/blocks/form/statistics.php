...
class Statistics
{
    ...
    public static function loadSurveys($MiniSurvey)
    {
        ...
        return $db->query('SELECT s.* FROM ' . $MiniSurvey->btTable . ' AS s, Blocks AS b, BlockTypes AS bt
            ...
            INNER JOIN CollectionVersions cv ON cvb.cID=cv.cID 
            AND cvb.cvID=cv.cvID AND 1=cv.cvIsApproved 
            AND (cv.cvPublishDate IS NULL OR cv.cvPublishDate <= ?) 
            AND (cv.cvPublishEndDate IS NULL OR cv.cvPublishEndDate >= ?)
